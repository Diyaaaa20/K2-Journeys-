    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: "#fff", color: "#1A1A2E", overflowX: "hidden" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .floating-label.active { top: -10px; transform: translateY(0); font-size: 11px; color: #0ABFBC; font-weight: 600; background: #fff; padding: 0 4px; left: 40px; }
      ...
      <section style={{ position: "relative", minHeight: 480, overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", padding: "72px 64px 60px", display: "flex", justifyContent: "space-between", gap: 40, alignItems: "center" }}>
          <div style={{ maxWidth: 520, animation: "fadeLeft 0.8s ease both" }}>
            ...
            <p style={{ fontFamily: "'Caveat', cursive", color: "#0ABFBC", fontSize: 26, fontWeight: 600, marginBottom: 28 }}>
            ...
            <div style={{ display: "flex", gap: 14 }}>
              {[{ label: "Chat With Us", icon: MessageCircle, primary: true }, { label: "View Destinations", icon: Globe, primary: false }].map(({ label, icon: Icon, primary }) => (
                <button key={label} style={{ background: primary ? "#0ABFBC" : "transparent", color: primary ? "#fff" : "#0D1321", fontWeight: 700, fontSize: 14, padding: "12px 22px", borderRadius: 24, border: primary ? "none" : "1.5px solid #D1D5DB", display: "flex", alignItems: "center", gap: 8, boxShadow: primary ? "0 4px 16px rgba(10,191,188,0.3)" : "none", transition: "all 0.2s" }} onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"} onMouseLeave={e => e.currentTarget.style.transform = ""}>
                  <Icon size={14} /> {label}
                </button>
              ))}
            </div>
      ...
      <section style={{ display: "flex", minHeight: 600 }}>
        <div ref={cardsRef} style={{ width: "38%", background: "#0D1321", padding: "64px 48px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <p style={{ color: "#0ABFBC", fontSize: 12.5, fontWeight: 700, letterSpacing: 3, marginBottom: 14, animation: cardsInView ? "fadeLeft 0.6s ease both" : "none" }}>REACH US</p>
          ...
          <div style={{ width: 44, height: 3, background: "#0ABFBC", marginBottom: 36, animation: cardsInView ? "fadeLeft 0.6s ease 0.15s both" : "none" }} />
            ...
            {CONTACT_CARDS.map((c, i) => (
              <div
                ...
                style={{
                  ...
                  border: `1px solid ${hoveredCard === i ? "#0ABFBC44" : "rgba(255,255,255,0.08)"}`,
        ...
        <div ref={formRef} style={{ flex: 1, background: "#F9FAFB", padding: "64px 52px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <div style={{ animation: formInView ? "fadeRight 0.7s ease 0.1s both" : "none" }}>
            ...
            <div style={{ width: 44, height: 3, background: "#0ABFBC", marginBottom: 32 }} />

            {sent && (
              <div style={{ background: "#E6F9F9", border: "1px solid #0ABFBC", borderRadius: 14, padding: "18px 20px", marginBottom: 24, display: "flex", alignItems: "center", gap: 12, animation: "fadeUp 0.4s ease" }}>
                <CheckCircle size={22} color="#0ABFBC" style={{ animation: "checkPop 0.4s ease", flexShrink: 0 }} />
                <div>
            ...
            <form onSubmit={handleSubmit}>
                ...
                {half.map(({ name, label, icon: Icon, type }) => {
                  ...
                  return (
                    <div key={name} className="field-group" style={{ position: "relative" }}>
                      <Icon size={15} color={focused === name ? "#0ABFBC" : "#9CA3AF"} style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)", transition: "color 0.2s", zIndex: 2, pointerEvents: "none" }} />
                      ...
                      <input
                        ...
                        style={{ width: "100%", padding: "16px 14px 16px 42px", borderRadius: 12, border: `1.5px solid ${focused === name ? "#0ABFBC" : "#E5E7EB"}`, fontSize: 14, color: "#0D1321", background: "#fff", boxShadow: focused === name ? "0 0 0 4px rgba(10,191,188,0.1)" : "none", transition: "all 0.25s", outline: "none" }}
                      />
                    </div>
                  );
                })}
              ...
              {full.map(({ name, label, icon: Icon, type }) => {
                ...
                return (
                  <div key={name} className="field-group" style={{ position: "relative", marginBottom: 16 }}>
                    <Icon size={15} color={focused === name ? "#0ABFBC" : "#9CA3AF"} style={{ position: "absolute", left: 16, top: isTextarea ? 16 : "50%", transform: isTextarea ? "none" : "translateY(-50%)", transition: "color 0.2s", zIndex: 2, pointerEvents: "none" }} />
                    ...
                    {isTextarea ? (
                      <textarea
                        ...
                        onFocus={() => setFocused(name)}
                        onBlur={() => setFocused(null)}
                        style={{ width: "100%", padding: "16px 14px 14px 42px", borderRadius: 12, border: `1.5px solid ${focused === name ? "#0ABFBC" : "#E5E7EB"}`, fontSize: 14, color: "#0D1321", background: "#fff", boxShadow: focused === name ? "0 0 0 4px rgba(10,191,188,0.1)" : "none", transition: "all 0.25s", outline: "none", resize: "none" }}
                      />
                    ) : (
                      <input
                        type={type}
              ...
              <button
                type="submit"
                style={{ width: "100%", background: "#0ABFBC", color: "#fff", fontWeight: 700, fontSize: 15, padding: "17px", borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 8, boxShadow: "0 6px 24px rgba(10,191,188,0.35)", transition: "all 0.25s" }}
      ...
      {/* CTA BANNER */}
      <section style={{ background: "#fff", padding: "56px 64px" }}>
        <div style={{ background: "linear-gradient(135deg, #0D1321 0%, #133048 60%, #0D1321 100%)", borderRadius: 24, padding: "48px 52px", display: "flex", alignItems: "center", gap: 36, position: "relative", overflow: "hidden" }}>
          ...
          <img src="https://images.unsplash.com/photo-1551632811-561732d1e306?w=200&h=200&fit=crop" alt="" style={{ width: 88, height: 88, borderRadius: "50%", objectFit: "cover", border: "3px solid #0ABFBC", flexShrink: 0, position: "relative" }} />
